...
- GitHub Actions runs on pushes and pull requests to `master`; it installs dependencies with `npm ci`, runs lint, runs the test suite, and performs a production security audit with `npm audit --audit-level=high`.
